    ...
    <td colspan="{{ $colspan }}" class="text-center">
        <div class="flex items-center text-center h-96">
            <div class="flex flex-col w-full max-w-sm px-4 mx-auto">
                ...
                <h1 class="mt-3 text-lg text-gray-800 dark:text-white">No data found</h1>
                @if ($searchValue || $FilterValue)
                    <p class="mt-2 text-gray-500 dark:text-gray-400">Your search did not match any
                        data.
                        Please try again or create add a new data.</p>
                @endif

                <div class="flex items-center mt-4 sm:mx-auto gap-x-3">
                    @if ($searchValue)
                        <button wire:click="{{ $stateSearch }}"
