    ...
    @if ($showTable)
        <section>
            <x-ui.table.header title="Capaian Pembelajaran Lulusan" wire-search="search">
                ...
                @endcan
                @can('create', [App\Models\SubCapaianPembelajaranMatakuliah::class, ['Kaprodi']])
                    <x-slot name="action">
            ...
            @php
                $showAction = Gate::check('create', [App\Models\SubCapaianPembelajaranMatakuliah::class, ['Kaprodi']]);
                $columnHeaders = ['No', 'Program Studi', 'Kode SUBCPMK', 'Deskripsi'];
                if ($showAction) {
            ...
            <x-ui.table.index :columns="$columnHeaders" :showAction="$showAction">
                @forelse ($data as $row)
                    <x-ui.table.row>
                        ...
                        @if (!Gate::allows('create', [App\Models\SubCapaianPembelajaranMatakuliah::class, ['Kaprodi']]))
