<div>
    ...
    <x-ui.pages.title title="Capaian Pembelajaran Lulusan" />
    @if ($showTable)
        <section>
            <x-ui.table.header title="Capaian Pembelajaran Lulusan" wire-search="search">
                <x-slot name="filter">
                    @can('filter', [\App\Models\CapaianPembelajaranLulusan::class, ['Kaprodi','Dosen']])
                        <flux:dropdown>
                ...
                @can('create', [\App\Models\CapaianPembelajaranLulusan::class, ['Kaprodi']])
                    <x-slot name="action">
            ...
            @php
                $columnHeaders = ['No', 'Program Studi', 'Kode CPL', 'Deskripsi'];
                if (Gate::allows('create', [App\Models\CapaianPembelajaranLulusan::class, ['Kaprodi']])) {
                    unset($columnHeaders[1]);
                }
            @endphp
            <x-ui.table.index :columns="$columnHeaders" :showAction="Gate::allows('create', [App\Models\CapaianPembelajaranLulusan::class, ['Kaprodi']])">
                @forelse ($data as $row)
                    <x-ui.table.row>
    ...
    @if ($showCreate)
        <div class="grid grid-cols-6 grid-rows-1 gap-4 mt-2">
            <div class="col-span-2 col-start-3">
                <livewire:master.cpl.create-update wire:key="create" />
            </div>
        </div>
    @endif
    ...
    @if ($showUpdate)
        <div class="grid grid-cols-6 grid-rows-1 gap-4 mt-2">
            <div class="col-span-2 col-start-3">
                <livewire:master.cpl.create-update wire:key="update-{{ $selectedId }}" :id="$selectedId" />
            </div>
        </div>
    @endif
